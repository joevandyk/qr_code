...
- [ ] **Database Schema:** Create a comprehensive database schema with:
...
- [ ] **Persistence:** Save the user's input to the database instead of just URL parameters, associate with a session token, and ensure data can be retrieved when using the back button.
- [ ] **Session Management:** Implement secure session handling with Phoenix tokens and ensure proper expiration and rotation.
...
2. Example test code:
   ...
   ```
3. Run `mix test test/qr_code_web/features/mockup_feature_test.exs` to generate screenshots
