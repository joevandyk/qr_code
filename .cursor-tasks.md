...
Notes: use the mishka components where possible
...
- [ ] **Route:** Add `live "/", HomeLive` to `QrCodeWeb.Router` (browser pipeline).
- [ ] **LiveView scaffold:** Generate `QrCodeWeb.HomeLive` with `mount/3`, default assigns, telemetry span `"home_live.mount"`.
- [ ] **Template:** Create `home_live.html.heex` with `<form phx-submit="next">` and single text input `name="url"`.
- [ ] **Styling:** Apply Tailwind classes (`w-full p-3 border rounded-lg focus:outline-none focus:ring`).
- [ ] **Accessibility:** Add `aria-label`, `aria-describedby` on input; insert visually‑hidden `<label>` for screen readers.
- [ ] **Autofocus:** Implement `phx-hook="Autofocus"` JS hook to focus field on mount.
- [ ] **Unit test:** Add LiveView test asserting form renders and pushes `"next"` event.
- [ ] **Docs:** Update README quick‑start with “Enter URL” screenshot.

### Story 2 – URL Validation
- [ ] **Validator module:** Create `QrCodeWeb.UrlValidator` with `validate/1` using `URI.parse/1` and regex for http/https.
- [ ] **Live validation:** Call `UrlValidator.validate/1` in `handle_event("next", ...)`.
- [ ] **Error feedback:** Add assign `:url_error`, render under input with red text and `role="alert" aria-live="polite"`.
- [ ] **Telemetry:** Emit event `[:qr_code, :url_validation, :failed]` with metadata `{url: url}`.
- [ ] **Unit tests:** Pass/ fail cases including edge inputs (`ftp://`, missing scheme).
- [ ] **i18n:** Add error strings to `gettext` catalogue.
- [ ] **Docs:** Update “Validation Rules” section.
