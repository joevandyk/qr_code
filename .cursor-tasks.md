...
- [x] **Template Display:** Create grid layout showing all available QR code template options.
- [x] **Selection Mechanism:** Implement template selection with visual indicator for the active template.
- [x] **Preview:** Show preview of how templates will look with the QR code.
- [x] **Style Options:** Include at least 4 template styles (Pop Vibes, Ocean Blue, Sunny Side, Monochrome).
- [x] **Navigation:** Add continue and back buttons for moving between create and preview pages.
- [x] **Basic QR Generation:** Implement simple QR code generation for preview purposes with watermark.
- [x] **Database Update:** Update the QR request record when a template is selected and load previously selected template when returning to this page.
- [x] **Tests:** Validate template selection and navigation flow.

## Epic 3 – Checkout

### Story 5 – Checkout Page [skip for now]
