...
config :lock_screen_qr_code, LockScreenQRCode.Repo,
  ...
  password: "postgres",
  hostname: "localhost",
  database: "lock_screen_qr_code_test#{System.get_env("MIX_TEST_PARTITION")}",
...
config :lock_screen_qr_code, LockScreenQRCode.Mailer, adapter: Swoosh.Adapters.Test
...
config :wallaby,
  ...
  # base_url: LockScreenQRCodeWeb.Endpoint.url(), # This might not work directly here
  js_logger: true

# Import test configuration from endpoint.exs
# import_config "../lib/lock_screen_qr_code_web/endpoint.ex" # Removed as it causes compile-time issues
